    ...
    <nav className="border-b border-slate-200 bg-white sticky top-0 z-50 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4">
        <div className="flex items-center justify-between flex-nowrap gap-4">
          ...
          {/* Navigation Links */}
          <div className="flex items-center gap-2 overflow-x-auto whitespace-nowrap md:overflow-visible">
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.path}
                  to={item.path}
                  className={`relative px-3 sm:px-4 py-2.5 rounded-lg font-medium transition-all ${
                    isActive
                      ? 'text-slate-800'
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            ...
            <Link
              to="/profile"
              className="p-2.5 bg-slate-100 hover:bg-slate-200 border border-slate-200 rounded-lg transition-all"
            >
            ...
            <button
              ...
              className="px-3 sm:px-4 py-2.5 bg-red-50 hover:bg-red-100 text-red-600 border border-red-200 rounded-lg font-medium flex items-center gap-2"
            >
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
